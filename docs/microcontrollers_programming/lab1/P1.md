...
El objetivo de esta sesión es introducir los conceptos básicos de la programación de microcontroladores y el control del tiempo. Aprenderemos a configurar pines digitales, a utilizar la estructura de un programa en Arduino (`setup()` y `loop()`) y a controlar LEDs y pulsadores mediante funciones como `delay()` y `millis()`. 
...
2. Instala la familia de placas M5Stack:
    ...
    ::: warning
    Por favor, ten en cuenta las actualizaciones recientes de la librería ESP32 Core para Arduino. En 2024, el núcleo Arduino-ESP32 se actualizó desde la versión 2.x (basada en ESP-IDF 4.4) a la versión 3.x (basada en ESP-IDF 5.1). La API subyacente y el sistema de compilación cambiaron varios comportamientos. Gran parte del código que puedas encontrar en internet fue escrito para la versión anterior 2.x. Todas las sesiones de laboratorio y los programas de este curso se han actualizado a la versión más reciente. Puedes encontrar más información sobre las versiones 2.x y 3.x [aquí](../arduino_esp32_core).
    :::
...
3. Instala las librerías de Arduino para M5Core2:
    ...
    ::: warning
    Cuando pulses *Install*, verás una lista de dependencias. *DEBES* instalar todas ellas también.
    :::
...
4. Instala el controlador CP2104 (controlador USB):
    ...
    - [Linux](https://m5stack.oss-cn-shenzhen.aliyuncs.com/resource/drivers/CP210x_VCP_Linux.zip)  (Descomprime el archivo y sigue las instrucciones de compilación en `CP210x_VCP_Linux_4.x_Release_Notes.txt`)
...
::: warning
- Algunos pines de M5Core2 están preconfigurados, así que presta atención al conectar componentes externos.
...
Conecta un LED de la siguiente manera:
...
- Usando el mismo circuito que en el ejercicio anterior, escribe un script que haga parpadear el LED a una frecuencia de 10 Hz.
...
- Usando el mismo circuito que en el ejercicio anterior, escribe un script que haga parpadear el LED a una frecuencia de 2 Hz y deje de parpadear después de 5 segundos.
...
- Simula el ejemplo de sketch:
    ...
    ::: warning
    Aquí estás *simulando* el comportamiento del sistema, incluyendo el código. No estás *compilando* el código, es decir, no estás generando el código legible por la máquina que ejecutará un procesador. Sin embargo, el código debe ser compilable, lo que significa que se verificará para asegurar que pueda compilar.
    :::
...
- Conecta un LED a `GPIO 21` con una resistencia, como se muestra en el diagrama (gira los componentes con `r` y giéralos con `p` si es necesario).
...
- Implementa el circuito que se muestra en el siguiente diagrama y simula un programa que:
...
- Usando el mismo circuito que en el ejercicio anterior, escribe un script que haga parpadear el LED mientras el botón está pulsado.
...
- Usando el mismo circuito que en el ejercicio anterior, escribe un script que haga lo siguiente:
